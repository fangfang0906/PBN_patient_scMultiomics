...
DimPlot(seu, group.by = "cell_type", reduction = 'umap',
...
```{r}
# Figure S1 ####
p1 <- DimPlot(seu, group.by = "predicted.celltype.l2", reduction = 'ref.umap',
        label = TRUE, label.size = 3,repel = T) + NoLegend()+
  labs(title='Project our dataset to Hao et al., 2021')
p2 <- DimPlot(ref, group.by = "celltype.l2", reduction = 'wnn.umap',
        label = TRUE, label.size = 3,repel = T) + NoLegend()+
  labs(title='Reference dataset from Hao et al., 2021')
p2/p1
FeaturePlot(seu, c("CCND1","CD79A","MS4A1","IGHM"),ncol=4)
```
...
meta <- seu@meta.data
asplit <- split(rownames(meta), meta$predicted.celltype.l2)
aggr <- do.call(cbind, lapply(names(asplit), function(x) {
  rowMeans(rna[,asplit[[x]]])
}))
colnames(aggr) <- names(asplit)
# reference
rna_ref <- ref@assays$SCT@scale.data
meta_ref <- ref@meta.data
meta_ref <- meta_ref[!(meta_ref$celltype.l2 %in% 'Doublet'),]
asplit_ref <- split(rownames(meta_ref), meta_ref$celltype.l2)
names(asplit_ref) <- paste0("ref_",names(asplit_ref))
aggr_ref <- do.call(cbind, lapply(names(asplit_ref), function(x) {
  rowMeans(rna_ref[,asplit_ref[[x]]])
}))
colnames(aggr_ref) <- names(asplit_ref)
ok <- intersect(rownames(aggr),rownames(aggr_ref))
merged <- cbind(aggr[ok,],aggr_ref[ok,])
cor <- cor(merged)
library(corrplot)
corrplot(cor[c(1:30),c(31:60)], tl.col = "black")
```
